...
async def main():
    ...
    ap.add_argument(
        '--fallback', choices=['searchapi'], default=None,
        help='retry blocked engines through a paid API (needs SEARCHAPI_KEY). '
             'Off unless asked for: it spends credits.'
    )

    args = ap.parse_args()

    fallback = fallback_for if args.fallback == 'searchapi' else None
    proxy = args.proxy
    ...
    if not engines:
        ...
        if 'all' in engines:
            engine = AllSearchEngines(proxy, timeout, fallback)
        elif len(engines) > 1:
            engine = MultipleSearchEngines(engines, proxy, timeout, fallback)
        else:
            stand_in = fallback(engines[0], proxy, timeout) if fallback else None
            engine = search_engines_dict[engines[0]](proxy, timeout, fallback=stand_in)
